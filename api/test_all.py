...
from api.repository import SQL_BASE, InMemoryTodoRepository, SQLTodoRepository, TodoRepository, get_engine
...
@pytest.mark.integration
def test_api_create(fake_todo_repository: TodoRepository, todo_repository: TodoRepository):
    time.sleep(1)
...
@pytest.mark.integration
def test_api_get_right(fake_todo_repository: TodoRepository, todo_repository: TodoRepository):
    time.sleep(1)
    client = TestClient(app)
    response = client.post("/create/testkey?value=testvalue")
...
@pytest.mark.integration
def test_api_wrong(fake_todo_repository: TodoRepository, todo_repository: TodoRepository):
    time.sleep(1)
    client = TestClient(app)
    response = client.post("/create/testkey?value=testvalue")
